...
export const getCategory = async (req: Request, res: Response) => {
  ...
  try {
    const [rows] = await pool.execute(
      'SELECT * FROM category WHERE uid IN(?,?)',
      [cateDefault,uid]
    );
...
export const deleteCategory = async (req: Request, res: Response) => {
  ...
  try {
    await pool.execute(
      'DELETE FROM category WHERE uid = ? AND uid != ? AND cate_id = ?',
      [uid, cateDefault, cate_id]
    );
...
export const updateCategory = async (req: Request, res: Response) => {
  ...
  try {
    await pool.execute(
      'UPDATE category SET cate_name = ?, cate_pic = ? WHERE uid = ? AND uid != ? AND cate_id = ?',
      [cate_name, cateDefault, cate_pic, uid, cate_id]
    );
